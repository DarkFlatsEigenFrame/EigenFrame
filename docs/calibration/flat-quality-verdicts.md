...
- **Reject N flickering.** The same reject, worded this way on a **Rotation groups** verdict, where the frames it names are the ones flagged for a flicker or a spike. It is cleanup of individual frames and stands apart from the rotation split, which it does not resolve either way.
- **Build split masters.** Offered on a **Rotation groups** verdict while the night is set to split. It opens the build panel for that night with the per-rotation split already checked, ready for you to review the selection and submit. It does not submit anything by itself.
- **Mark optical-train change.** Only offered on a **Boundary candidate** verdict. Confirming it records the date as the point your optical train changed, which is what tells EigenFrame that a flat from one side of this point never calibrates a light from the other. Read [how long a flat is good for](./flat-epochs.md) for what that boundary actually does.
